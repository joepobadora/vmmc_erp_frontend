<script>
    ...
    $effect(() => {
        p.set('page', tablePage).set('search', filter.search).set('status', filter.status);
    });
    ...
    async function refreshTable() {
        ...
        try {
            const result = await App.API.post('/admin/document-types', {
                filter: filter,
            });
    ...
    <j.Col>
        <nav style="--bs-breadcrumb-divider: '>';">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item small active">Document Types</li>
            </ol>
        </nav>
    </j.Col>
    <j.Col auto>
        {#if $permissions.includes('ADMIN.DOCTYPES_CREATE')}
...
<j.Card>
    ...
    {:else}
        <Table data={docTypes} enableTotalCount enablePagination pageSize="10" bind:currentPage={tablePage}>
            <div slot="row" let:item class="row border-bottom custom-row small">
                <div class="col">
                    <div>
                        <span class="text-muted me-2">Type:</span>
                        {#if $permissions.includes('ADMIN.DOCTYPES_VIEW')}
                            <strong
                                class="custom-link"
                                onclick={() => {
                    ...
                    <div>
                        {#if $permissions.includes('ADMIN.DOCTYPES_EDIT')}
                            <span
